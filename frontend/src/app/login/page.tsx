...
export default function LoginPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!clinicRes.ok) {
        throw new Error(clinicData.error || 'Failed to initialize clinic profile')
      }
      ...
      if (data.user) {
        ...
        if (!userInsert.ok) {
          ...
          throw new Error(userInsertData.error || 'Failed to link user profile to database')
        }
  ...
  return (
    <main className="min-h-screen bg-slate-950 flex flex-col justify-center items-center p-6 relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md bg-slate-900/60 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-8 shadow-2xl z-10">
        ...
        <div className="flex flex-col items-center mb-8">
          <div className="w-12 h-12 rounded-xl bg-gradient-to-tr from-[#D4AF37] to-amber-500 flex items-center justify-center shadow-lg shadow-[#D4AF37]/10 mb-3">
            <span className="text-slate-950 font-black text-xl">B</span>
          </div>
          <h1 className="text-2xl font-bold text-white tracking-tight">BassirAI</h1>
          <p className="text-slate-400 text-xs mt-1">Aesthetic Medical Communication Engine</p>
        </div>
        ...
        <form onSubmit={isSignUp ? handleSignUp : handleLogin} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-[#D4AF37] to-amber-500 hover:from-amber-500 hover:to-[#D4AF37] text-slate-950 font-bold text-sm rounded-lg py-3 mt-4 transition-all duration-300 transform active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none flex items-center justify-center"
          >
            {loading ? (
