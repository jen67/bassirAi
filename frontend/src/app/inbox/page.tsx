...
const MOCK_THREADS: ChatThread[] = [
  {
    ...
    messages: [
      {
        sender: "patient",
        text: "Hi, I want to book Botox for next week Thursday",
        time: "10:15 AM",
      },
      {
        sender: "ai",
        text: "Hello Chioma! Botox treatments at Zuri Clinic range from ₦180,000 to ₦300,000. I can schedule a slot for you. Do you prefer morning or afternoon?",
        time: "10:16 AM",
      },
      {
        sender: "patient",
        text: "Morning is fine, but I have a special skin condition. Can I speak to a doctor?",
        time: "10:18 AM",
      },
    ],
  },
  {
    ...
    messages: [
      {
        sender: "patient",
        text: "Where is your clinic located in Lekki?",
        time: "Yesterday",
      },
      {
        sender: "ai",
        text: "We are located in Lekki Phase 1, Lagos. We provide free parking validation right in front of the clinic entrance for all patients!",
        time: "Yesterday",
      },
    ],
  },
  {
    ...
    messages: [
      ...
      {
        sender: "ai",
        text: "Lip Fillers (Juvederm) at Zuri Clinic cost ₦450,000 - ₦600,000 per syringe depending on density. Would you like to schedule a session?",
        time: "Yesterday",
      },
    ],
  },
...
export default function InboxPage() {
  ...
  const simulateInbound = async (text: string, isArabic = false) => {
      ...
      if (!activeThread.takeover) {
        setTimeout(() => {
          ...
          if (isArabic) {
            ...
            text.toLowerCase().includes("laser") ||
            text.toLowerCase().includes("resurfacing")
          ) {
            aiReply =
    ...
    try {
      ...
      const { error: patErr } = await supabase.from("messages").insert({
        clinic_id: clinicId,
      ...
      if (patErr) {
        console.error(
          "Failed to log inbound simulation message:",
          patErr.message,
        );
        return;
      }
  ...
  const simulateOutboundHuman = async (text: string) => {
    ...
    try {
      // Use new /api/messages/send endpoint which integrates with n8n
      ...
      if (!response.ok) {
        ...
        console.error("Failed to send doctor reply:", errorData.error);
      }
      ...
      setTimeout(async () => {
        await supabase.from("messages").insert({
          ...
          conversation_id: activeThread.id,
          content:
            "Yes, Thursday at 10:00 AM works perfectly for me! See you then. Thank you!",
          direction: "inbound",
          is_ai_generated: false,
        });
      }, 2000);
  ...
  const simulateNewConversation = async () => {
    ...
    try {
      ...
      if (convErr) {
        console.error(
          "Failed to create new simulated conversation:",
          convErr.message,
        );
        return;
      }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Use new /api/messages/send endpoint which handles n8n integration
      const response = await fetch("/api/messages/send", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      <div className="h-[calc(100vh-130px)] flex flex-col md:flex-row gap-6 font-sans">
        ...
        <div className="w-full md:w-80 bg-slate-900/40 border border-slate-900/60 rounded-2xl flex flex-col overflow-hidden shrink-0">
          <div className="p-4 border-b border-slate-850 flex justify-between items-center">
            <h2 className="text-sm font-bold tracking-wider text-[#D4AF37] uppercase">
              Conversations
            </h2>
        ...
        <div className="flex-1 bg-slate-900/40 border border-slate-900/60 rounded-2xl flex flex-col overflow-hidden">
          {threads.length === 0 ? (
            <div className="flex-1 flex flex-col justify-center items-center text-slate-500 text-xs gap-4 p-8 text-center bg-slate-950/20">
              ...
              <div>
                <p className="font-bold text-slate-400">
                  Your Database Inbox is Empty
                </p>
                <p className="text-[10px] text-slate-500 mt-1 max-w-xs mx-auto">
                  No live conversations found. Click below to simulate an
                  inbound WhatsApp patient connection inside your live Supabase
                  database.
                </p>
              </div>
              <button
            ...
            <>
              ...
              <div className="p-4 bg-slate-900/60 border-b border-slate-850 flex justify-between items-center">
                ...
                <div className="flex items-center gap-4">
                      ...
                      <div className="absolute right-0 mt-2 w-64 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl z-20 py-2 divide-y divide-slate-800">
                        ...
                        <div className="py-1">
                          ...
                          <button
                            onClick={() => {
                              simulateInbound(
                                "Do you offer skin laser treatments?",
                                false,
                              );
                              setShowSimMenu(false);
                            }}
                          ...
                          <button
                            onClick={() => {
                              simulateOutboundHuman(
                                "Hi there! This is Dr. Benson. I've reviewed your request. Let's schedule a session for Thursday morning. Does 10:00 AM work?",
                              );
                              setShowSimMenu(false);
                            }}
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    >
                      <div
                        className={`w-5 h-5 rounded-full bg-slate-950 transition-transform transform ${
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col gap-1 max-w-[75%]">
                        ...
                        <span className="text-[9px] text-slate-500 self-end pr-1 mt-0.5">
                          {msg.time}
                        </span>
                      </div>
                    </div>
              ...
              <div className="p-4 bg-slate-900/60 border-t border-slate-850 flex flex-col gap-2">
                {/* Show hint when AI mode is active */}
                {!activeThread.takeover && (
                  <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg py-1.5 px-3 flex items-center gap-2 text-[10px] text-blue-400">
                    ...
                    💡 AI is currently responding. Type a message to
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-950 border border-slate-850 rounded-xl px-4 py-2.5 text-xs text-white placeholder-slate-700 focus:outline-none focus:border-[#D4AF37]"
                  />
                  <button
